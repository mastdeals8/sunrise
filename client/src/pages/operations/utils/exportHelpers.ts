...
/** Client-side Excel export — no Express backend needed. Uses xlsx-js-style. */
export async function exportEstimateToExcel(
  ...
  const tax = items.reduce((s, it) => s + num(it.cgstAmount ?? it.cgst_amount) + num(it.sgstAmount ?? it.sgst_amount) + num(it.igstAmount ?? it.igst_amount), 0);
